      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/10 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white mt-1">
            People &amp; Youth Control Center
          </h1>
          <p className="text-xs text-gray-400 mt-0.5">
            Managing private Supabase candidate records &amp; issued Civic
            Passports.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            >
              ...
              <option value="Public Policy & Governance">
                Public Policy &amp; Governance
              </option>
              <option value="Research & Empirical Analytics">
                Research &amp; Empirical Analytics
              </option>
              <option value="Technology & AI Engineering">
                Technology &amp; AI Engineering
              </option>
              <option value="Editorial & Journalism">
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {filteredApps.map((app) => (
                <div
                  key={app.id}
                  className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4 hover:border-cyan-500/40 transition-all"
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-white">
                        {app.total_experience_years || 'Fresher'}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500 block font-mono text-[10px]">
                  ...
                  {app.cover_note && (
                    <div className="bg-[#0b1228] p-3 rounded-xl border border-white/5 text-xs text-gray-300">
                      <span className="text-gray-500 block font-mono text-[10px] mb-1">
                        Statement of Intent:
                      </span>
          ...
          {passports.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {passports.map((pass) => (
                <div
                  key={pass.id}
                  className="bg-white/5 border border-amber-500/30 rounded-2xl p-5 space-y-2"
                >
                  ...
                  <p className="text-xs text-gray-400">
                    {pass.email} • {pass.phone || 'N/A'}
                  </p>
                  <p className="text-[11px] text-gray-500 font-mono">
                    Payment ID: {pass.payment_id} ({pass.amount_paid})
                  </p>
                </div>
              ))}
            </div>
