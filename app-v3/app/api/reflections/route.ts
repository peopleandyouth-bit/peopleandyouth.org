...
// Force dynamic execution so Next.js build step skips static pre-evaluation
...
export async function POST(request: Request) {
  try {
    ...
    // 3. Build Institutional HTML Email
    const htmlEmail = `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <style>
          ...
          .section-title { font-size: 14px; font-weight: 800; color: #fbbf24; margin-top: 28px; margin-bottom: 12px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 6px; text-transform: uppercase; letter-spacing: 1px; }
    ...
    // 4. Dispatch via Resend API safely inside POST handler
    ...
    if (apiKey) {
      try {
        ...
        console.log(`✅ Resend email dispatched to ${author_email}`);
      } catch (resendErr: any) {
